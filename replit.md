...
## Recent Changes
- **2025-10-28**: **CRITICAL FIX - Eliminated Speculation Language**: Strengthened prediction accuracy analysis prompt to eliminate ALL speculation words ("likely", "probably", "may have", "might have", "appears to", "could have", etc.). System prompt changed to enforce definitive statements only. Temperature lowered to 0.1 for pure factual analysis. Added 10 banned words with explicit before/after examples including user's specific complaint ("The prediction likely overestimated" → "The prediction overestimated - he scored X pts"). Analysis now provides only provable facts from exact FPL data.
