...
## Recent Changes
- **2025-10-24**: Fixed critical UX issue where app displayed "Current Gameweek GW 8" but gave recommendations for GW 10 (the next editable gameweek). App now correctly uses FPL's `is_next` flag to identify the planning gameweek. All pages updated to show "Planning for GW X" instead of "Current Gameweek", accurately reflecting that recommendations are for the next editable gameweek (since current gameweek locks after deadline). Updated across Dashboard, Gameweek Planner, and Team Modeller.
- **2025-10-23**: ✅ PRODUCTION READY - Continuity-aware AI recommendation system fully tested and deployed. AI now maintains consistency across multiple plan generations unless there's a significant data change (injury confirmations, suspensions, major form shifts ≥1.0, fixture changes). Minor fluctuations (±0.1 form, ±0.05 xG, ±0.1m price) no longer trigger recommendation changes. When AI does change recommendations, it provides explicit reasoning comparing current vs previous data. UI displays amber alert for changes (with reasoning) or green alert for maintained continuity (fully tested). This allows users to plan ahead with confidence while remaining responsive to important mid-week updates. Database schema updated with `recommendationsChanged` (boolean) and `changeReasoning` (text) fields. Green continuity path verified in production, amber change path code-complete and ready for real FPL data changes.
- **2025-10-23**: Fixed AI prediction infinite loop and achieved perfect determinism. Root causes: (1) Team Modeller useEffect had `analyzeMutation` in dependency array causing continuous re-triggering, (2) Missing `seed` parameter allowed wording variations despite temperature: 0. Solutions: Removed mutation from dependencies, added `seed: 42` to all 7 GPT-4o calls. Result: Predictions now trigger ONCE per change and produce word-for-word identical results for same team composition (verified: 48 pts, 85% confidence, identical insights text across all requests).
- **2025-10-23**: Migrated AI prediction system from GPT-5 to GPT-4o with temperature: 0 and seed: 42 for perfect deterministic predictions. GPT-5 does not support temperature: 0 (locked at 1, causing variance), while GPT-4o with both temperature: 0 and seed enables consistent recommendations for identical input data. This ensures users receive the exact same predictions unless actual FPL data changes (form, fixtures, injuries).
...
- **AI:** OpenAI GPT-4o with temperature: 0 and seed: 42 for perfect deterministic predictions (via Replit AI Integrations)
...
- **AI Prediction Pipeline**: User input processed by Context Builder, then GPT-4o (temperature: 0, seed: 42) for perfect deterministic analysis, returning structured and natural language responses. AI services cover player points prediction, transfer recommendations, captain selection, chip strategy, and team analysis.
