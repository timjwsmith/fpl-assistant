...
## Recent Changes
- **2025-10-23**: Implemented continuity-aware AI recommendation system. AI now maintains consistency across multiple plan generations unless there's a significant data change (injury confirmations, suspensions, major form shifts ≥1.0, fixture changes). Minor fluctuations (±0.1 form, ±0.05 xG, ±0.1m price) no longer trigger recommendation changes. When AI does change recommendations, it provides explicit reasoning comparing current vs previous data. UI displays amber alert for changes (with reasoning) or green alert for maintained continuity. This allows users to plan ahead with confidence while remaining responsive to important mid-week updates.
