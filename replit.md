...
- **Deterministic Budget Validation System**: Transfer validation uses actual selling prices from FPL API (`purchase_price` and `selling_price` from picks endpoint) instead of approximating with current market prices. This eliminates budget calculation errors where AI recommended unaffordable transfers. The system stores per-player purchase/selling prices in the database and enforces strict validation that blocks any transfer plan exceeding available funds. Legacy squads without price data are rejected with a clear "Refresh Team" instruction until user resyncs. Budget formula: `bank + Σ(selling_price_out) - Σ(now_cost_in)` (all in tenths).
