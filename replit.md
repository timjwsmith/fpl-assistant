...
## Recent Changes
- **2025-10-27**: **PHASE 1: Unified Data Layer Architecture** - Implemented `GameweekDataSnapshot` service to eliminate data inconsistency issues. All features (predictions, transfers, captain recommendations, gameweek planning, analysis) now consume data from a single canonical snapshot with timestamp tracking. This prevents situations where different screens showed different player availability or stats. System logs now show snapshot timestamps for debugging. Benefits: (1) Consistent data across all features, (2) Reduced API calls through shared caching, (3) Easier to debug (know exactly what data was used), (4) Foundation for coordinated AI pipeline in Phase 2.
